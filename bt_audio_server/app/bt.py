...
class BTDevice(collections.namedtuple("BTDevice", ["mac_address", "name", "select_url", "paired"])):

	@classmethod
	def from_btctl_result(cls, d, paired):
		try:
			url = url_for("api.api_select_bt_device", mac=d["mac_address"])
		except RuntimeError:
			url = ""

		return cls(d["mac_address"], d["name"], url, paired)
...
#	devices = thread.last_scan_result()
#
#	if len(devices):
#		for d in devices:
...
#
#	if thread.pair_count():
#		print("Connecting to {}".format(thread.paired_devices[0].name))
#		if thread.connect_paired(thread.paired_devices[0].mac_address):
#			print("Connected!")
#		else:
